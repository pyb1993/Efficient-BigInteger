...
void FFTFunctor::fft(const RefVec Coef, RefVec FFT, short sign){
	...
	for (size_t interval = 1; interval < FFTLength; interval*= 2) // 执行 log2(nn) 次外循环
		...
		//cout << "outer loop\n";
		    ...
		    double OmegaR = 1;
		    double OmegaI = 0;
		    for (register size_t m = 0;  m < interval; ++m)//m代表每一个子变换的起始位置
			    {
				for (register size_t i = m; i < FFTLength; i += 2*interval)// FFT[i]代表每一个子序列在当前位置的FFT系数
				    {
				        auto j = i + interval;
...
void FFTFunctor::InverseFFT(const RefVec FFT, RefVec Coef)
{
	...
	for (size_t i = 0; i <2 * len; i += 2) 
		Coef[i / 2] = (floor(0.5 + invNFFT*IFFT[i]));/* 四舍五入,还原序列*/
}
